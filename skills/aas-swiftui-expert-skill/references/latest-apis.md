...
> **Refresh check (2026-07-20):** Re-scanned every category in `references/scan-manifest.md` via the Sosumi MCP (Apple-docs rendering, which exposes `deprecated: true` + "Use X instead"). Result: **no new deprecations found** beyond the entries already listed below. All documented modern replacements (foregroundStyle, MagnifyGesture/RotateGesture, Tab, @Observable/@Bindable, @Entry, glassEffect, WebView, dragContainer, @Animatable, confirmationDialog, etc.) were re-verified as still current. The Sosumi MCP was enabled in this Hermes profile at `https://sosumi.ai/mcp` (no API key). Note: the manifest's iOS 26 "rumor" candidates (navigationBarTitleDisplayMode, toolbarTitleDisplayMode, presentationBackground, presentationDetents, searchable, scrollTargetBehavior) are confirmed still current, not deprecated.
